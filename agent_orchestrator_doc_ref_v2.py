...
try:
    if faiss and os.path.exists("faiss_index.bin"):
        ...
        print(f"{Fore.YELLOW}⚠️ FAISS not available or index file missing.{Style.RESET_ALL}")
...
def retrieve_chunks(query, k=3):
    if not index or not metadata:
        return [f"[Mock Doc] Relevant info for '{query}' (no FAISS index available)"]
...
def query_llm(prompt):
    if not bedrock:
        return f"[MOCK RESPONSE] Here's an example answer for: {prompt[:60]}..."
    ...
    try:
        ...
        if "content" in response_body:
            if isinstance(response_body["content"], list):
                return response_body["content"][0].get("text", "[No text found]")
...
def detect_action(llm_response):
    match = re.search(r"\[ACTION: (\w+)\]", llm_response)
    return match.group(1) if match else None


def main():
    print(f"{Fore.CYAN}🤖 C3PO at your service! Type your question (or 'exit' to quit).{Style.RESET_ALL}")
    while True:
